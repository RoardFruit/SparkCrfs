...
import breeze.linalg.{norm => brzNorm, axpy => brzAxpy}
...
class SimpleUpdater extends Updater {
  override def compute(
    ...
    brzAxpy(-thisIterStepSize, gradient, weightsOld)
...
class L1Updater extends Updater {
  override def compute(
    ...
    var i = 0
    val len = weightsOld.length
    while (i < len) {
      val wi:Double= weightsOld(i)
      weightsOld(i) = signum(wi) * max(0.0, abs(wi) - shrinkageVal)
      i += 1
    }

    (weightsOld, brzNorm(weightsOld, 1.0) * regParam)
  }
}
...
class SquaredL2Updater extends Updater {
  override def compute(
    ...
    brzAxpy(-thisIterStepSize, gradient, weightsOld)
    val norm = brzNorm(weightsOld, 2.0)
